...
<div class="container">
    ...
    <input name="task_name" id="task_name_input" type="text" placeholder="Wprowadź nazwę zadania"/>
    <button counting="0" class="timer-button">Start</button>
    <div class="timer_history">
        <?php foreach ($data['archived_tasks'] as $archived_task) {
            $interval = date_diff(date_create($archived_task['start_date']), date_create($archived_task['end_date']));
            $date = new DateTime($archived_task['start_date']);
        ?>
            <div class="timer_history_entry">
                <p class="timer_entry_title"><?php echo $archived_task['task_name']; ?></p>
                <p><?php echo $date->format('Y-m-d'); ?></p>
                <p><?php echo $interval->format('%H:%I:%S'); ?></p>
                <a href="/edit?id=<?php echo $archived_task["id"]?>">Edycja</a>
                <a href="/delete?id=<?php echo $archived_task["id"]?>" style="color:#E86548">X</a>
            </div>
            <?php
        }
        ?>
